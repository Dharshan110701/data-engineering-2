...
class MyOptions(PipelineOptions):
    ...
    def _add_argparse_args(cls, parser):
        parser.add_argument('--input', help='Input file path on GCS', required=True)
        parser.add_argument('--output', help='Output BigQuery table', required=True)
...
def run():
    ...
    with beam.Pipeline(options=pipeline_options) as p:
        (
            p
            | "Read Entire JSON File" >> beam.Create([read_entire_file(options.input)])
